      ...
      >
        <div role="search" :class="getClassWrapper">
          <input
            ...
            autocomplete="off"
            required="required"
            :class="getClassInputPlaceholder"
            tabindex="-1"
          />
          <input
            :disabled="disabled"
          ...
          <button
            ...
            type="submit"
            :class="getClassSubmit"
            tabindex="-1"
          >
            <svg role="img" aria-label="Search">
              <use
                xmlns:xlink="http://www.w3.org/1999/xlink"
                :xlink:href="getSVGSearch"
              ></use>
            </svg>
          </button>
          <button
...
export default {
  ...
  data() {
    return {
      ...
      highlightedIndex: 0,
      highlightedIndexMax: this.$props.maxSuggestions,
      similiarData: [],
  ...
  methods: {
    ...
    arrowDownValidation() {
      return (
        this.highlightedIndex < this.highlightedIndexMax &&
    ...
    findRepited(similarItem, o) {
      return (
        similarItem[this.suggestionAttribute] === o[this.suggestionAttribute]
      );
    },
    ...
    finalTextValueValidation() {
      return (
        typeof this.similiarData[this.highlightedIndex] !== "undefined" ||
            ...
            words.forEach(function (word) {
              if (textValWords.length > 0) {
                textValWords.forEach(function (textValWord) {
                  if (
                    word.toLowerCase().startsWith(textValWord.toLowerCase())
                  ) {
                    isMatch = true;
                  }
                });
              } else if (
                word.toLowerCase().startsWith(this.textVal.toLowerCase())
              ) {
                isMatch = true;
              }
            });
    ...
    notArrowKeysEvent() {
      return (
        this.selectedEvent !== "ArrowUp" &&
        this.selectedEvent !== "ArrowDown" &&
        this.selectedEvent !== "ArrowRight"
      );
    },
    notEnterKeyEvent() {
